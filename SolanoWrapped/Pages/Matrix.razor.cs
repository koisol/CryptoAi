...
namespace SolanoWrapped.Pages
{
    public partial class Matrix
    {
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
                await JS.InvokeVoidAsync("initializeMatrixAnimation");
                loadScenarios();
            }
        ...
        private async Task Search()
        {
            ...
            if (!MemoryCache.TryGetValue(TokenInput, out string cachedMessage)) 
            { cachedMessage = GetRandomMessage();
                MemoryCache.Set(TokenInput, cachedMessage); 
            }
